...
const LabelModal = ({ isLabelModalOpen, setIsLabelModalOpen, onOpenPrint, activeProduct }) => {
    ...
    >
      <div ref={panelRef} tabIndex={-1} className={`${theme.bodyBg} border ${theme.border} rounded-3xl shadow-2xl shadow-black/50 w-full max-w-4xl max-h-[90vh] overflow-y-auto relative animate-in fade-in zoom-in duration-300 focus:outline-none`}>
        ...
        <div className="bg-black/40 p-6 border-t border-white/10 flex flex-col md:flex-row justify-between items-start md:items-center text-xs text-gray-400 rounded-b-3xl gap-4">
          ...
          <button onClick={onOpenPrint} className={`shrink-0 bg-black/40 border ${theme.buttonBorder} ${theme.buttonText} hover:bg-white/5 font-bold py-3 px-5 rounded-xl shadow-sm transition-colors flex items-center gap-2 w-full sm:w-auto justify-center`}>
