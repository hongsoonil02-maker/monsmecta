...
const PrintModal = ({ isPrintModalOpen, setIsPrintModalOpen, activeProduct }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm transition-opacity" role="dialog" aria-modal="true" aria-labelledby="print-modal-title">
      <div className={`bg-slate-100 rounded-3xl shadow-2xl w-full ${isOriginal ? 'max-w-4xl' : 'max-w-[1250px]'} max-h-[90vh] overflow-y-auto relative animate-in fade-in zoom-in duration-300 flex flex-col`}>
        {/* Modal Header */}
        ...
        <div className={`flex-1 w-full flex justify-center items-center bg-gray-900 rounded-b-3xl overflow-hidden ${isOriginal ? 'min-h-[600px]' : 'min-h-[500px]'}`}>
          <iframe 
            src={`${import.meta.env.BASE_URL}assets/labels/${activeProduct}_label_print.html`}
            className={`w-full ${isOriginal ? 'h-[800px] scale-75' : 'h-[600px] scale-100'} border-none bg-white origin-top`}
            title="Print Preview"
          />
        </div>
      </div>
